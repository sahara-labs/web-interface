...
Globals.RC_CONTROLLER = "WindTunnelController";

/** {String} Cookie prefix. */
Globals.COOKIE_PREFIX = "wind-";
 ...
 */
WindTunnel.prototype.setup = function() {
	var o;
	...
	/* -- Fan controls -------------------------------------------------------- */
	o = new SwitchWidget(this.$container, "Fan Enable", "Fan On", "fan", "motor-enable", "setMotor");
    o.setDraggable(true);
    this.widgets.push(o);
	
	o = new SliderWidget(this.$container, "Fan", "fan", "motor-speed", "setMotor");
 ...
 * Runs the interface. 
 */
WindTunnel.prototype.run = function() {
	/* Render the page. */
	this.display.init();
...
WindTunnel.prototype.acquireLoop = function() {
	var thiz = this;
 ...
 */
WindTunnel.prototype.processData = function(data) {
 ...
 */
WindTunnel.prototype.errorData = function(msg) {    
	if (!this.dataError)
	{
...
TabbedWidget.prototype.init = function() {
	...
	if (this.isResizable) this.enableResizable(this.width, this.height, this.keepAspect);
};
...
TabbedWidget.prototype.switchTab = function() {
    ...
    if (this.isResizable)
    {
        this.widgets[this.currentMode].resized(this.width, this.height);
        this.widgets[this.currentMode].resizeStopped(this.width, this.height);
    }
...
TabbedWidget.prototype.resized = function(width, height) {
    this.width = width;
    this.height = height;
    
    this.$widget.find(".tab-content").css({
        width: width + 1,
        height: height + 1
    });
    
    /* Bubble resize to inner widget. */
    if (this.currentMode !== undefined) this.widgets[this.currentMode].resized(width, height);
};

TabbedWidget.prototype.resizeStopped = function(width, height) {
    this.width = width;
    this.height = height;
    
    /* Bubble resize to inner widget. */
    if (this.currentMode !== undefined) this.widgets[this.currentMode].resizeStopped(width, height);
};
...
/**
 * Sets whether the tab widget is resizable.
 * 
 * @param resizable whether the widget is resizable
 * @param keepAspect whether to keep the aspect ratio
 */
...
function GraphWidget($container, title, chained) 
{
	...
	/** ID of canvas. */
	this.id = "graph-" + title.toLowerCase().replace(' ', '-').replace('/', '');
...
CameraWidget.VID_WIDTH_DIFF = 0;

/** Difference between widget height and video height. */
CameraWidget.VID_HEIGHT_DIFF = 35;
